...
2026-07-20 데이터 가능성 시험에서 이 표본을 `U0 Legacy-50`으로 명명하고 50/50 공식 식별을 완료했다. `U1 Expanded-10`은 과거 시점별 10개 업종 소속, 상장폐지·합병·분할·코드 변경 계보 및 기업행사 조정 수익률 경로가 모두 공식 자료로 확인되기 전에는 V2.1에 포함하지 않는다. 현재 U1은 `NOT_ADOPTED`이며 V2.2 후보이다. `SECT_TP_NM`은 공식 산업분류로 추정하지 않는다.
