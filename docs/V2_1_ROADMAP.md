...
연구 프로토콜 상태는 `DRAFT_NOT_FROZEN`, 저장소 상태는 `PORTFOLIO_RELEASE_COMPLETE`다. V2.1 전체 target, 모델, Walk-forward 예측, 성능, 백테스트는 생성하지 않았다.

`v0.2.0`은 2025 연구 재구성, V2 기준선, V2.1 사전등록과 데이터 가능성 시험, U0 수집기 골격을 완결된 포트폴리오 산출물로 공개한다. 아래 Gate 3 이후는 현재 릴리스의 누락 작업이 아니라, 외부 공식 근거를 확보한 뒤 재개할 후속 연구다.
...
여기서 `release-blocking`은 **V2.1 실증 결과 공개를 차단한다는 뜻**이다. 코드·프로토콜·허용된 집계 metadata만 포함한 `v0.2.0` 포트폴리오 릴리스는 원자료와 행 단위 파생 결과를 포함하지 않으므로 완료할 수 있다.
...
5. frozen manifest를 만든 뒤 전체수집과 품질 보고를 먼저 수행한다.
6. target과 모델 결과는 수집 품질 gate 통과 후 사전등록 순서대로 한 번 생성한다.

공식 답변이 확보되지 않으면 `v0.2.0`을 현재 프로젝트의 최종 공개본으로 유지한다. 이는 실패나 미완성이 아니라, 확인할 수 없는 데이터 정의로 실증 결과를 만들지 않겠다는 연구 판단이다.
