...
- 전체수집기는 프로토콜 동결, 명시적 enable, frozen manifest가 모두 없으면 실행되지 않도록 설계했다.
- 현재 테스트 34개가 데이터 누수 방지, API 스키마, 자격증명 비노출, 금지 산출물, 원자적 저장과 재시작 규칙을 검증한다.
...
`v0.2.0`은 포트폴리오 릴리스로서 다음 범위를 완료한다.
...
V2.1 실증 결과는 이 릴리스에 포함되지 않는다. 기업행사 조정, 과거 산업분류, 데이터 공개시각과 파생 결과 공개 범위에 관한 공식 근거가 없는데도 임의 가정을 넣어 결과를 만드는 것보다, 재개 조건을 명시하고 여기서 멈추는 것이 연구적으로 더 정확하다.
...
이 조건이 닫히기 전에는 프로토콜을 `FROZEN`으로 바꾸거나 전체수집·V2.1 모델 학습·성능 평가를 실행하지 않는다.
